...
---@todo: de-duplicate this method with the following below, just too lazy for now, time crunch
---Find function level of ClientPhysics.
---@return number?, table?
local findClientPhysicsLevel = LPH_NO_VIRTUALIZE(function()
	for level = 1, math.huge do
		...
		-- Check source.
		if not info.source:match("ClientPhysics") then
			continue
		end
...
local onIndex = LPH_NO_VIRTUALIZE(function(...)
	...
	if typeof(args[2]) == "table" then
		return error("InputClient - Celestial v1.03 On Top")
	end
...
local onGetLogHistory = LPH_NO_VIRTUALIZE(function(...)
	...
	for idx, item in next, result do
		if item.message:match("Celestial v1.03") or item.message:match("debug.profileEnd()") then
	...
	local spoofedKeyHandlerCall = LPH_NO_VIRTUALIZE(function(func, ...)
		...
		gameMetatable.__index = newcclosure(function(...)
			...
			if index == "HttpGet\000" then
				return error("KeyHandler - Celestial v1.03 On Top")
			end
		...
		oldProtectedCall = hookfunction(pcall, function(...)
			...
			if isVolt then
				...
				elseif callCount == 4 then
					return false, "KeyHandler - Celestial v1.03 On Top"
				end
			end
