-- Detach and initialize a Celestial v1.03 instance.
local Celestial = { queued = false, silent = false, dpscanning = false, norpc = false }
...
-- Celestial v1.03 maid.
local celestialMaid = Maid.new()
...
function Celestial.init()
	...
	if isfile and isfile("smarker.txt") then
		Celestial.silent = true
	end

	if isfile and isfile("dpscanning.txt") then
		Celestial.dpscanning = true
	end

	if isfile and isfile("norpc.txt") then
		Celestial.norpc = true
	end

	--[[
	if script_key and queue_on_teleport and not Celestial.queued and not no_queue_on_teleport then
		-- String.
	...
	if Celestial.norpc then
		return
	end

	bloxstrapRPCModule.SetRichPresence({
		details = "Celestial v1.03 (Attached)",
		state = string.format(
	...
	local playerRemovingSignal = celestialMaid:mark(Signal.new(playersService.PlayerRemoving))

	playerRemovingSignal:connect("Celestial_OnLocalPlayerRemoved", function(player)
...
function Celestial.detach()
	...
	if bloxstrapRPCModule then
		bloxstrapRPCModule.SetRichPresence({
			details = "Celestial v1.03 (Detached)",
...
-- Return the Celestial v1.03 module.
return Celestial
